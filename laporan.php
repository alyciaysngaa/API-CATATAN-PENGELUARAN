<?php 
	...
	$today = $year . '-' . $month . '-' . $day;
     ...
     $start = $_GET['start'] ?? date('Y-m-d'); // Nilai default adalah tanggal hari ini
     $end = $_GET['end'] ?? date('Y-m-d'); // Nilai default adalah tanggal hari ini
 
     // Validasi tanggal (opsional)
     $start = date('Y-m-d', strtotime($start)); // Ini memastikan format tanggal yang benar
     $end = date('Y-m-d', strtotime($end)); // Ini memastikan format tanggal yang benar
 
     $type = $_GET['type'] ?? 'pengeluaran'; // Jika 'type' tidak diberikan, gunakan 'pengeluaran' sebagai default
     $data;
 
     if ($type == 'pengeluaran') {
         $data = query("SELECT * FROM pengeluaran WHERE tanggal BETWEEN '$start' AND '$end' AND username = '$ambilNama'");
     } else {
         $data = query("SELECT * FROM pemasukkan WHERE tanggal BETWEEN '$start' AND '$end' AND username = '$ambilNama'");
     }
...
<body>
    <div class="header">
        ...
        <h3 class="text-secondary font-weight-bold float-left logo">Wallet</h3>
        <h3 class="text-secondary float-left logo2">Q</h3>
        <a href="logout">
            <div class="logout">
            ...
            <div class="konten_dalem khusus3">
                ...
                <div class="table-responsive">
                    <table class="laporan">
                        <tr>
                            ...
                            <td colspan="3">
                                <select id="jenis-laporan" class="form-control">
                                    <option value="pemasukkan" <?= $type === 'pemasukkan' ? 'selected' : '' ?>>Pemasukkan</option>
                        ...
                        <tr>
                            <td>Pilih tanggal</td>
                            <td><input type="date" id="awal" class="form-control control" value="<?= $start ?>"></td>
                            <td>sampai</td>
                            <td><input type="date" id="akhir" class="form-control control" value="<?= $end ?>"></td>
                            <td><button class="btn btn-primary lapor" onclick="showReport()">Tampilkan</button></td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="tampil">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= htmlspecialchars($_GET['type'] ?? null) ?></td>
    ...
    <script src="keuangan/js/laporan.js"></script>
    <script src="js/bootstrap.js"></script>
    <script>
        function showReport() {
            const jenis = document.getElementById('jenis-laporan').value;
            const start = document.getElementById('awal').value;
            const end = document.getElementById('akhir').value;
            window.location.href = `?type=${jenis}&start=${start}&end=${end}`;
        }
    </script>
</body>
